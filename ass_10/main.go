...
func generateInitialPopulation(inst *Instance, seed int64, numberSol int) Population {
	...
	for len(population.Solutions) < numberSol {
		...
		for len(population.Solutions) < numberSol {
			...
			tour0, in0 := RandomStart(inst, runRnd)
			// rundomized greedy start

			var finalTour []int
			if rand.Float64() < 0.8 {
				finalTour, _ = GreedyRegretStart(inst, tour0[0])
			} else {
...
func runMethods(inst *Instance, runs int, seed int64, outPath string) error {
	...
	for _, m := range method {
		...
		for i, tour := range population.Solutions {
			...
			obj := calculationObjective(inst, population.Solutions[bestidx])
			cost := SelectedCosts(inst.Nodes, population.Solutions[bestidx])
			strSel := make([]string, len(population.Solutions[bestidx]))
			for i := range population.Solutions[bestidx] {
				strSel[i] = strconv.Itoa(population.Solutions[bestidx][i])
			}
			...
			if err := w.Write([]string{
				...
				strconv.Itoa(i + 1),
				strconv.Itoa(obj),
				strconv.Itoa(len(population.Solutions[bestidx])),
				strconv.Itoa(cost),
				strconv.Itoa(0),
				strconv.Itoa(0),
				strings.Join(strSel, ";"),
